...
func (b *executorBuilder) build(p plannercore.Plan) Executor {    //executor 中的build才是真正的physical plan builder
	switch v := p.(type) {
	case nil:
		return nil
...
/*
 * 我们需要自定义buildHashJoin,当他发现下面两个孩子节点是基表扫描，并且这个扫描
 * 都是来自postgresql,那么我们就不去进行孩子的递归构架table reader exector了
 */
...
func (b *executorBuilder) buildHashJoin(v *plannercore.PhysicalHashJoin) Executor {
	...
	leftChild := childs[0]
	rightChild := childs[1]
	leftChildPh, ok1 := leftChild.(*plannercore.PhysicalTableReader)       //这个是表的reader
	rightChildPh, ok2 := rightChild.(*plannercore.PhysicalTableReader)
	if ok1 && ok2{
		//说明找到了两个孩子，都是基表的扫描，先别急，再看看他们是不是来自同一个postgreSQL数据源的
		leftChildSource := leftChildPh.SourceType
		rightChildSource := rightChildPh.SourceType
		if leftChildSource=="postgresql" && rightChildSource=="postgresql"{
			//可以直接构造一个执行器了，将这join操作，下推给pg
			leftRPCInfo := leftChildPh.Path
			rightRPCInfo := rightChildPh.Path
			lpaths :=  strings.Split(leftRPCInfo, "#")
			rpaths := strings.Split(rightRPCInfo, "#")

			//然后将path进行比较host和port端口一样，才能认为连个表来自同一个pg实例
			if lpaths[0]==rpaths[0]&&lpaths[1]==rpaths[1]{
				ltablename := lpaths[2]
				rtablename := rpaths[2]

				lReaderPlan := leftChildPh.TablePlans[0]     //这个才是表的具体的scan计划
				rReaderPlan := rightChildPh.TablePlans[0]
				ltablescan, ok3 := lReaderPlan.(*plannercore.PhysicalTableScan)  //应该都是全表扫描的计划
				rtablescan, ok4 := rReaderPlan.(*plannercore.PhysicalTableScan)
				if ok3 && ok4{   //却保所有的表都是基表扫描
					lcol := ltablescan.Columns
					rcol := rtablescan.Columns
					//解析左表的属性
					lattr := ""
					for i, col := range lcol{
						if i>0{
							lattr+=","
						}
						//lattr+=col.Name      两表的名字会重叠
						lattr += ltablename+"."+col.Name.L
					}
					//解析右表的属性
					rattr := ""
					for i, col := range rcol{
						if i>0{
							rattr+=","
						}
					...
					//这个join的条件，这里似乎拉取不出信息来，需要早expression中加个函数
					if joinCondition!=nil{
						onstring = expression.JoinEqExpressionToString(joinCondition, ltablename, rtablename)
					}
					// ok 条件解析完毕，我们直接反解析一条SQL



					/*
					 * 需要考虑的是，有些谓词在计划阶段已经被下推了，所以我们要把他们捞上来
					 * 这是一个bug，所以有了以下的操作
					 */
					...
					//sql := "select " + attr + "from" + ltablename + ", " + rtablename + " where "+onstring
					sql := ""
					if onstring != ""{
						sql = "select "+attr+" from "+ltablename+","+rtablename+" where "+onstring
						//需要考虑的是，有些谓词在计划阶段已经被下推了，所以我们要把他们捞上来
						...
						sql = "select "+attr+" from "+ltablename+","+rtablename
						flag := 1
						if leftChildPh.PushDownCondition!=""{
					...
					e := &PushDownJoinExec{
						baseExecutor:newBaseExecutor(b.ctx, v.Schema(), v.ExplainID()),
						PushDownJoinSQL:sql,
						Results:v.Schema().Columns,    //按照代码的意思，这里的col，已经被投影过了？？？
						PathInfo:lpaths[0]+"#"+lpaths[1]}
					return e
				}
			}
		}
	...
	//后面是hash join本身的代码，递归遍历孩子节点，进行executor的构造
