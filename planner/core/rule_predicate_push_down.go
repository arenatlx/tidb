...
func (s *ppdSolver) optimize(lp LogicalPlan) (LogicalPlan, error) {      //逻辑计划到逻辑计划
...
func addSelection(p LogicalPlan, child LogicalPlan, conditions []expression.Expression, chIdx int) {
	...
	selection := LogicalSelection{Conditions: conditions}.Init(p.context())   //在当前节点和child节点添加selection节点
	selection.SetChildren(child)
	p.Children()[chIdx] = selection
}

// PredicatePushDown implements LogicalPlan interface.        逻辑计划树中进行遍历，从root到叶子，谓词一层层的下去
func (p *baseLogicalPlan) PredicatePushDown(predicates []expression.Expression) ([]expression.Expression, LogicalPlan) {
	if len(p.children) == 0 {
		return predicates, p.self
	}
	child := p.children[0]
	rest, newChild := child.PredicatePushDown(predicates)           //这里的谓词下推，就是在孩子节点中写入从条件信息，节点本身不变
	addSelection(p.self, newChild, rest, 0)           //条件下推之后，以防万一，我还是在这里做个条件的selection
	return nil, p.self
}
...
func (ds *DataSource) PredicatePushDown(predicates []expression.Expression) ([]expression.Expression, LogicalPlan) {
	...
	//postgresql可以进行谓词下推工作
	//进行逻辑计划的改造，但是当我们是csv文件时候，我们不做谓词下推，不做logical plan的变化，返回原来的logical plan
	...
	if ds.SourceType=="redis"{                     //这里这将expression数据根据数据源选择性下推
		...
		//最后才根据pg的，pg需要表名，来构造下推的string，所以传入了tableNamex的表名字
		ds.PushDownConditionString = expression.ExpressionToString(ds.pushedDownConds,tableNamex)
	}
