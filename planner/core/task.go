...
//////////////////////////////////////////CSV, postgresql, redis都是使用的csv的task
...
func attachPlan2Task(p PhysicalPlan, t task) task {
	...
	case *csvTask:
		p.SetChildren(v.p)     //将这个task，添加为物理计划的孩子
		v.p = p
	case *rootTask:
		p.SetChildren(v.p)
		v.p = p
