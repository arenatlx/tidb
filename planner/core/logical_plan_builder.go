...
func (b *PlanBuilder) buildResultSetNode(node ast.ResultSetNode) (p LogicalPlan, err error) {
	...
	case *ast.TableSource:                          //AST node中，能够成为表名字，不一定只有表源啊，也是select as语句等等
		switch v := x.Source.(type) {
		case *ast.SelectStmt:
			p, err = b.buildSelect(v)
		case *ast.UnionStmt:
			p, err = b.buildUnion(v)
		case *ast.TableName:                        //同样的，ast中是语法解析而来的过程，并不具有格式化的source，path信息
			p, err = b.buildDataSource(v)           //我们需要在逻辑计划build的过程，对表名节点，进行内部限制级SQL的操作，将
		default:                                    //将source，path信息，填充在datasource逻辑节点中
...
func (b *PlanBuilder) buildJoin(joinNode *ast.Join) (LogicalPlan, error) {
	...
	if joinNode.Right == nil {                            //joinnode下面的右边节点为空的话，那么这个hash join节点就没有用了
		return b.buildResultSetNode(joinNode.Left)
	}

	b.optFlag = b.optFlag | flagPredicatePushDown         //上层可能还有谓词，叠加使用，看看能不能下推

	leftPlan, err := b.buildResultSetNode(joinNode.Left)   //此处同理
	if err != nil {
		return nil, errors.Trace(err)
	}
	...
	if err != nil {
		return nil, errors.Trace(err)
                ...
                //这边有一种编程模式，joinPlan中的logicalSchemaProducer中有baseLogicalPlan，去遍历它的孩子
