...
func (e *Explain) explainPlanInRowFormat(p PhysicalPlan, taskType, indent string, isLastChild bool) {
	/*
	 * 因为这是一个递归解释计划的过程，switch中，我们强行使他从我们的sourceType中，taskType类型进行了自定义的赋值
	 * 这里，我们还想，将hashjoin的操作，同样给它一个执行的地点（因为一般只有tablescanreader才有会有tasktype，
	 * tikv一般是cop类型，之前我们又加了csv和postgreSQL，现在我们相对hashjoin节点，也进行tasktype的显示)
	 */
	...
	case *PhysicalTableReader:{      //这边是解释信息的酷炫操作
