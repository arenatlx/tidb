...
func (expr *ScalarFunction) ExplainInfo() string {       //函数
	...
	buffer.WriteString(")")     //函数的信息就是func("string",int)这种形式的info
	return buffer.String()
}

// ExplainInfo implements the Expression interface.
func (expr *Column) ExplainInfo() string {   //类似db.table.colum这种的形式
	return expr.String()
}
...
func (expr *Constant) format(dt types.Datum) string {
	...
	case types.KindString, types.KindBytes, types.KindMysqlEnum, types.KindMysqlSet,
		...
		return fmt.Sprintf("\"%v\"", dt.GetValue())   //字符串，用"xxx"表示
	}
	return fmt.Sprintf("%v", dt.GetValue())         //其他类型直接返回xxx就行了
}
